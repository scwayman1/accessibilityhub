...
| Clerk owner identity | Networkless RS256, issuer/time/azp/v2/session/subject checks; email-only, impersonated, and Organization-context auth are rejected | Production instance, PEM public key, issuer, invitation acceptance, bound `user_…` ID, live positive/negative tests | Development settings hardened; invitation/production held |
...
| Activation gate | Exact-true flag, manifest version, fresh runtime evidence bound to the configured verification-run ID, and an unconditional missing-handler blocker | Reviewed implementation/Blueprint release plus all evidence below | Locked in code |
...
| Queue/worker | Clean-only deterministic job envelope; dormant locked worker imports no network or processing clients and receives no live-service credential; lifecycle transition contract | Private Render Key Value, resource/time limits, and infrastructure-enforced outbound-denial proof | Dormant worker ready; Render environment isolation does not block public egress, so processing remains blocked |
| Audit | Code and database action allowlists reject content/OCR/prompts/responses/tokens/credentials/URLs; row mutation and truncation triggers | Separate authoritative protected sink, non-owner INSERT-only writer role, retention/access policy, recovery test | Metadata outbox foundation complete; authoritative sink pending |
| Manual deletion | Inventory covers quarantine, clean, derivatives, evidence, jobs, findings, consents, and document; absence proof and durable immutable verified tombstone required | Storage/database deletion adapters, tombstone-aware restore, backup-expiry SLA, live positive/negative tests | Contract/runbook/schema guard complete; adapters/SLA pending |
...
| Locked infrastructure definition | Separate non-default Blueprint pinned to `main`; protected network-isolated environment; previews/service auto-deploys off; runbook requires dashboard Auto Sync off; maintenance mode; exact custom domain; Render subdomain disabled; predeploy guard checks these locks | Billing approval, merged source, Blueprint Auto Sync check, resource creation, and deployed inspection | Source validation passes; application paused before billing |
