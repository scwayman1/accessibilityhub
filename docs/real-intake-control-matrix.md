...
| Queue/worker | Clean-only deterministic job envelope; no external egress permission; lifecycle transition contract | Private Render Key Value, background worker, resource/time limits, network no-egress proof | Contract complete; queue/worker pending |
