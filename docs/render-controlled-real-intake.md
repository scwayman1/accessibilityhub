...
5. **Optional private scanner service** — only if the selected malware scanner must receive jobs over the private network. It has no public URL.
...
- A malware-scanning service or scanner image with a documented clean, rejected, and indeterminate verdict contract.
...
- Unsafe, oversized, malformed, and indeterminate-scan uploads are rejected before parsing.
...
- Backup/deletion timing and incident ownership are documented.
